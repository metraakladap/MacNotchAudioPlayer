...
public final class DynamicNotch<Expanded, CompactLeading, CompactTrailing>: ObservableObject, DynamicNotchControllable where Expanded: View, CompactLeading: View, CompactTrailing: View {
    ...
    ///
    /// Hover changes are recorded even while `state == .hidden`: transitions pass
    /// through a brief intermediate hidden state, and dropping a mouse-exit event
    /// during that window would leave `isHovering` stuck at `true` (keeping the
    /// notch expanded forever). Only the haptic feedback is gated on visibility.
    public func updateHoverState(_ hovering: Bool) {
        // Ensure that we only update when the state changes
        guard hovering != isHovering else { return }

        isHovering = hovering

        if hoverBehavior.contains(.hapticFeedback), state != .hidden {
        ...
        if needsNewWindow {
            ...
            Task { @MainActor in
                if !skipHide {
                    ...
                    // If something else (e.g. a compact() racing this expand)
                    // changed the state during the sleep, don't stomp it.
                    guard self.state == .hidden else { return }
                }
